...
def plot_arrow (ax, x, y, length, c):
    halfl = 0.5*length
    ax.arrow (x, y-halfl, dx=0, dy=2*halfl, width=0.045, length_includes_head=True, head_width=0.2, head_length=0.5*abs(halfl), fc=c, ec=c, capstyle='butt')
    #prop = dict(arrowstyle="->, head_width=0.3, head_length=1", facecolor=c, ec=c, linewidth=3.5)
...
def plot_circle (ax, x, y, h, hmax=1, hmin=0, circle_max=0.35, valcutoff=1e-3, color1='springgreen', color2='orange', **args):
    if abs(h) > valcutoff:
