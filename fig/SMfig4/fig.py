...
fig,axs = plt.subplots (ncols=4, nrows=4, figsize=(5.5,4.5),# sharex='col',
                        ...
                        wspace=0, hspace=0, left=0.03, bottom=0.11,right=0.85,top=0.97))
...
#for ax in [axs[3,1],axs[3,2],axs[3,3]]:
...
ps.text (axs[1,0], 0.5, 0.6, '$4$', fontsize=12)
ps.text (axs[2,0], 0.5, 0.6, '$6$', fontsize=12)
ps.text (axs[3,0], 0.5, 0.6, '$8$', fontsize=12)
ps.text (axs[0,1], 0.5, 0.5, '$16$', fontsize=12)
ps.text (axs[0,2], 0.5, 0.5, '$24$', fontsize=12)
ps.text (axs[0,3], 0.5, 0.5, '$32$', fontsize=12)
ps.text (axs[0,1], 0.07, 0.5, '$L_x$', fontsize=14)
ps.text (axs[1,0], 0.5, 0.89, '$L_y$', fontsize=14)
holecolor = "k"
dmrghole = "r"
...
for lx in Lx:
    for ly in Ly:
        ...
        if (lx==6 and (ly==16 or ly==24)) or (lx==8 and ly==16 ):
            tempax.text(0,0.157,"*",fontsize=20)
        ...
        tempax.errorbar(range(1,ly+1),hole,holeerr,color=holecolor,linewidth=0.4,marker='o',markersize=1)

        if lx<=6:
            dmrg = np.loadtxt("dmrg"+str(lx)+str(ly)+".dat")[::lx]
            for i in range(ly):  dmrg[i,3] *= (-1)**(i+1)
            tempax.plot(range(1,ly+1),dmrg[:,2],color=dmrghole,linewidth=0.3,linestyle='--',marker='+',markersize=1)
            

        tempax.axhline (color='grey',linestyle='--',linewidth=0.5)

        tempax.set_ylim([0.03,0.19])
